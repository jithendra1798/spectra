...
import { getTimeline } from "../adaptive/timelineStore";
...
export function Debrief() {
  ...
  const raw = getTimeline(sessionId);

  const data = useMemo(() => {
    // normalize x to "t0 + seconds"
  ...
  // phase boundaries: first occurrence of each phase
  ...
  return (
    <div style={{ padding: 24, color: "var(--text)" }}>
          ...
          {summary.peakStress ? (
            <>
              ...
              <div style={{ fontSize: 12, color: "var(--muted)" }}>
                {summary.peakStress.phase} · {fmtTime(summary.peakStress.t)}
              </div>
            </>
          ) : (
        ...
        <div className="card" style={{ padding: 14 }}>
          ...
          {summary.mostFocus ? (
            <>
              ...
              <div style={{ fontSize: 12, color: "var(--muted)" }}>
                {summary.mostFocus.phase} · {fmtTime(summary.mostFocus.t)}
              </div>
            </>
          ) : (
            <div style={{ fontSize: 12, color: "var(--muted)" }}>No data</div>
      ...
      <div style={{ marginTop: 18, fontSize: 12, color: "var(--muted)" }}>
        Next: we’ll add a second Claude call to summarize the run from the timeline data.
      </div>
    </div>
  );
}